...
export const dict = {
  zh: {
    ...
    twoSuffixNote:
      "註：台灣的債券 ETF 均在櫃買中心（TPEx）掛牌，代號後綴為 .TWO 而非 .TW；本平台已一併校正其計價幣別（新台幣）與對標基準（0050）。台股代號在表格、圖例與排名中一律附註基金名稱並省略交易所後綴（如 00687B(國泰20年美債)）—— 這些代號彼此往往只差一碼，只看數字極易看錯。",
    trendNote: "長期向上趨勢指標",
    trendNoteText:
  ...
  en: {
    ...
    twoSuffixNote:
      "Note: Taiwan's bond ETFs all list on TPEx and carry a .TWO suffix rather than .TW; their currency (TWD) and benchmark (0050) are handled accordingly. Taiwan tickers appear throughout with their fund name attached and the exchange suffix dropped — 00687B(Cathay US Treasury 20+ YR ETF TWD) — because these codes differ from one another by a single digit.",
